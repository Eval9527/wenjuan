...
export function HomeQuickGenerateForm() {
  ...
  return (
    <form action="/new" method="get" onSubmit={() => setIsSubmitting(true)} className="flex items-center gap-3 rounded-xl border border-[#d7dee8] bg-white p-2 shadow-sm transition-shadow focus-within:border-[#2563eb] focus-within:ring-4 focus-within:ring-[#2563eb]/10 hover:shadow-md">
      ...
      <button type="submit" className="ui-btn ui-btn-primary shrink-0 px-6" disabled={isSubmitting}>
        {isSubmitting ? <span aria-hidden="true" className="home-loading-dot home-loading-dot--light" /> : null}
        {isSubmitting ? '正在创建...' : '生成问卷'}
      </button>
    </form>
  );
}
